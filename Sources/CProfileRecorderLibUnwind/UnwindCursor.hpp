...
extern "C" _swipr_Unwind_Reason_Code __libunwind_seh_personality(
    int, _swipr_Unwind_Action, uint64_t, _swipr_Unwind_Exception *,
    struct _swipr_Unwind_Context *);
...
public:
  ...
  static void iterateCacheEntries(void (*func)(swipr_unw_word_t ip_start,
                                               swipr_unw_word_t ip_end,
                                               swipr_unw_word_t fde, swipr_unw_word_t mh));
...
public:
  ...
  virtual swipr_unw_word_t getReg(int) { _LIBUNWIND_ABORT("getReg not implemented"); }
  virtual void setReg(int, swipr_unw_word_t) {
    _LIBUNWIND_ABORT("setReg not implemented");
  }
  virtual bool validFloatReg(int) {
    _LIBUNWIND_ABORT("validFloatReg not implemented");
  }
  virtual swipr_unw_fpreg_t getFloatReg(int) {
    _LIBUNWIND_ABORT("getFloatReg not implemented");
  }
  virtual void setFloatReg(int, swipr_unw_fpreg_t) {
    _LIBUNWIND_ABORT("setFloatReg not implemented");
  }
  virtual int step(bool = false) { _LIBUNWIND_ABORT("step not implemented"); }
  virtual void getInfo(swipr_unw_proc_info_t *) {
  ...
  virtual bool getFunctionName(char *, size_t, swipr_unw_word_t *) {
...
/// an unwind.  This is normally stack-allocated inside a swipr_unw_cursor_t.
template <typename A, typename R>
class UnwindCursor : public AbstractUnwindCursor {
  typedef typename A::pint_t pint_t;
public:
                      UnwindCursor(swipr_unw_context_t *context, A &as);
                      UnwindCursor(CONTEXT *context, A &as);
                      UnwindCursor(A &as, void *threadArg);
  virtual             ~UnwindCursor() {}
  virtual bool        validReg(int);
  virtual swipr_unw_word_t  getReg(int);
  virtual void        setReg(int, swipr_unw_word_t);
  virtual bool        validFloatReg(int);
  virtual swipr_unw_fpreg_t getFloatReg(int);
  virtual void        setFloatReg(int, swipr_unw_fpreg_t);
  ...
  virtual void        jumpto();
  virtual bool        isSignalFrame();
  virtual bool        getFunctionName(char *buf, size_t len, swipr_unw_word_t *off);
  ...
  void setDispatcherContext(DISPATCHER_CONTEXT *disp) {
    _dispContext = *disp;
    _info.lsda = reinterpret_cast<swipr_unw_word_t>(_dispContext.HandlerData);
    if (_dispContext.LanguageHandler) {
      _info.handler = reinterpret_cast<swipr_unw_word_t>(__libunwind_seh_personality);
    } else
      _info.handler = 0;
  }
...
template <typename A, typename R>
UnwindCursor<A, R>::UnwindCursor(swipr_unw_context_t *context, A &as)
    : _addressSpace(as), _unwindInfoMissing(false) {
  static_assert((check_fit<UnwindCursor<A, R>, swipr_unw_cursor_t>::does_fit),
                "UnwindCursor<> does not fit in swipr_unw_cursor_t");
  static_assert((alignof(UnwindCursor<A, R>) <= alignof(swipr_unw_cursor_t)),
                "UnwindCursor<> requires more alignment than swipr_unw_cursor_t");
...
template <typename A, typename R>
swipr_unw_word_t UnwindCursor<A, R>::getReg(int regNum) {
  switch (regNum) {
#if defined(_LIBUNWIND_TARGET_X86_64)
  case UNW_X86_64_RIP:
...
template <typename A, typename R>
void UnwindCursor<A, R>::setReg(int regNum, swipr_unw_word_t value) {
  switch (regNum) {
#if defined(_LIBUNWIND_TARGET_X86_64)
  case UNW_X86_64_RIP:
...
template <typename A, typename R>
swipr_unw_fpreg_t UnwindCursor<A, R>::getFloatReg(int regNum) {
#if defined(_LIBUNWIND_TARGET_ARM)
...
template <typename A, typename R>
void UnwindCursor<A, R>::setFloatReg(int regNum, swipr_unw_fpreg_t value) {
#if defined(_LIBUNWIND_TARGET_ARM)
  if (regNum >= UNW_ARM_S0 && regNum <= UNW_ARM_S31) {
    union {
      uint32_t w;
  ...
  int stepWithEHABI() {
    ...
    const uint32_t *ehtp =
        decode_eht_entry(reinterpret_cast<const uint32_t *>(_info.unwind_info),
                         &off, &len);
    if (_swipr_Unwind_VRS_Interpret((_swipr_Unwind_Context *)this, ehtp, off, len) !=
...
bool UnwindCursor<A, R>::getInfoFromEHABISection(
  ...
  swipr_unw_word_t personalityRoutine = 0xbadf00d;
  bool scope32 = false;
  uintptr_t lsda;
  ...
  if (exceptionTableData & 0x80000000) {
    ...
    switch (choice) {
      case 0:
        personalityRoutine = (swipr_unw_word_t) &__aeabi_unwind_cpp_pr0;
        extraWords = 0;
        scope32 = false;
        lsda = isSingleWordEHT ? 0 : (exceptionTableAddr + 4);
        break;
      case 1:
        personalityRoutine = (swipr_unw_word_t) &__aeabi_unwind_cpp_pr1;
        extraWords = (exceptionTableData & 0x00ff0000) >> 16;
        scope32 = false;
        lsda = exceptionTableAddr + (extraWords + 1) * 4;
        break;
      case 2:
        personalityRoutine = (swipr_unw_word_t) &__aeabi_unwind_cpp_pr2;
        extraWords = (exceptionTableData & 0x00ff0000) >> 16;
        scope32 = true;
        lsda = exceptionTableAddr + (extraWords + 1) * 4;
        break;
...
bool UnwindCursor<A, R>::getInfoFromFdeCie(
  ...
  if (CFI_Parser<A>::parseFDEInstructions(_addressSpace, fdeInfo, cieInfo, pc,
    ...
    _info.extra             = static_cast<swipr_unw_word_t>(dso_base);
    return true;
  }
  return false;
}
...
bool UnwindCursor<A, R>::getInfoFromSEH(pint_t pc) {
  ...
  _info.unwind_info = reinterpret_cast<swipr_unw_word_t>(unwindEntry);
  _info.extra = base;
  ...
  if (pc != getLastPC()) {
    ...
    if (xdata->Flags & (UNW_FLAG_EHANDLER|UNW_FLAG_UHANDLER)) {
      ...
      _info.lsda = reinterpret_cast<swipr_unw_word_t>(handler+1);
...
enum frameType : swipr_unw_word_t {
  frameWithXLEHStateTable = 0,
  frameWithEHInfo = 1
};

extern "C" {
typedef _swipr_Unwind_Reason_Code __xlcxx_personality_v0_t(int, _swipr_Unwind_Action,
                                                     uint64_t,
                                                     _swipr_Unwind_Exception *,
                                                     struct _swipr_Unwind_Context *);
...
bool UnwindCursor<A, R>::getInfoFromTBTable(pint_t pc, R &registers) {
  ...
  swipr_unw_word_t start_ip =
      reinterpret_cast<swipr_unw_word_t>(TBTable) - *p - sizeof(uint32_t);
  swipr_unw_word_t end_ip = reinterpret_cast<swipr_unw_word_t>(TBTable);
  ++p;
  ...
  swipr_unw_word_t lsda = 0;
  swipr_unw_word_t handler = 0;
  swipr_unw_word_t flags = frameType::frameWithXLEHStateTable;
  ...
  if (TBTable->tb.lang == TB_CPLUSPLUS && TBTable->tb.has_ctl) {
    ...
    handler = reinterpret_cast<swipr_unw_word_t>(xlcPersonalityV0);
  ...
  _info.flags = flags;
  _info.format = 0;
  _info.unwind_info = reinterpret_cast<swipr_unw_word_t>(TBTable);
  _info.unwind_info_size = 0;
...
void UnwindCursor<A, R>::setInfoBasedOnIPRegister(bool isReturnAddress) {
  ...
  // This matches the behaviour of _swipr_Unwind_GetIP on arm.
  pc &= (pint_t)~0x1;
#endif
...
template <typename A, typename R>
void UnwindCursor<A, R>::getInfo(swipr_unw_proc_info_t *info) {
  if (_unwindInfoMissing)
...
#if defined(_LIBUNWIND_USE_CET) || defined(_LIBUNWIND_USE_GCS)
extern "C" void *__libunwind_cet_get_registers(swipr_unw_cursor_t *cursor) {
