...
_swipr_Unwind_Reason_Code ProcessDescriptors(
    _swipr_Unwind_State state,
    _swipr_Unwind_Control_Block* ucbp,
  ...
  while (descriptorWord) {
    ...
    uintptr_t pc = _swipr_Unwind_GetIP(context);
...
static _swipr_Unwind_Reason_Code unwindOneFrame(_swipr_Unwind_State state,
                                          _swipr_Unwind_Control_Block* ucbp,
                                          struct _swipr_Unwind_Context* context) {
  ...
  const char *lsda =
      reinterpret_cast<const char *>(_swipr_Unwind_GetLanguageSpecificData(context));

  // Handle descriptors before unwinding so they are processed in the context
  // of the correct stack frame.
  _swipr_Unwind_Reason_Code result =
  ...
  switch (__swipr_unw_step(reinterpret_cast<swipr_unw_cursor_t *>(context))) {
...
// Generates mask discriminator for _swipr_Unwind_VRS_Pop, e.g. for _UVRSC_CORE /
// _UVRSD_UINT32.
uint32_t RegisterMask(uint8_t start, uint8_t count_minus_one) {
  return ((1U << (count_minus_one + 1)) - 1) << start;
}

// Generates mask discriminator for _swipr_Unwind_VRS_Pop, e.g. for _UVRSC_VFP /
// _UVRSD_DOUBLE.
...
_swipr_Unwind_VRS_Interpret(_swipr_Unwind_Context *context, const uint32_t *data,
  ...
  while (offset < len && !finish) {
    ...
    if ((byte & 0x80) == 0) {
      uint32_t sp;
      _swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32, &sp);
      if (byte & 0x40)
        sp -= (((uint32_t)byte & 0x3f) << 2) + 4;
      else
        sp += ((uint32_t)byte << 2) + 4;
      _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32, &sp);
    } else {
      switch (byte & 0xf0) {
        case 0x80: {
          if (offset >= len)
          ...
          if (registers & (1 << 15))
            wrotePC = true;
          _swipr_Unwind_VRS_Pop(context, _UVRSC_CORE, registers, _UVRSD_UINT32);
          break;
        }
        case 0x90: {
          ...
          if (reg == 13 || reg == 15)
            return _URC_FAILURE;
          uint32_t sp;
          _swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_R0 + reg,
                          _UVRSD_UINT32, &sp);
          _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32,
                          &sp);
          break;
        }
        case 0xa0: {
        ...
        case 0xb0: {
          switch (byte) {
            ...
            case 0xb2: {
              ...
              uint32_t sp;
              _swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32,
                              &sp);
              sp += 0x204 + (addend << 2);
              _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32,
                              &sp);
            ...
            case 0xb3: {
              ...
              _swipr_Unwind_VRS_Pop(context, _UVRSC_VFP,
            ...
            case 0xb4:
              ...
              _swipr_Unwind_VRS_Pop(context, _UVRSC_PSEUDO,
        ...
        case 0xc0: {
          switch (byte) {
            ...
            case 0xc4:
            case 0xc5:
              _swipr_Unwind_VRS_Pop(context, _UVRSC_WMMXD,
            ...
            case 0xc7: {
              ...
              if (!v || v & 0xf0)
                return _URC_FAILURE;
              _swipr_Unwind_VRS_Pop(context, _UVRSC_WMMXC, v, _UVRSD_DOUBLE);
              break;
            }
#endif
            case 0xc8:
        ...
        case 0xd0: {
          if (byte & 0x08)
            return _URC_FAILURE;
          _swipr_Unwind_VRS_Pop(context, _UVRSC_VFP, RegisterRange(8, byte & 0x7),
                          _UVRSD_DOUBLE);
          break;
        }
        default:
  ...
  if (!wrotePC) {
    uint32_t lr;
    _swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_LR, _UVRSD_UINT32, &lr);
#ifdef __ARM_FEATURE_PAUTH
    if (hasReturnAddrAuthCode) {
      uint32_t sp;
      uint32_t pac;
      _swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32, &sp);
      _swipr_Unwind_VRS_Get(context, _UVRSC_PSEUDO, 0, _UVRSD_UINT32, &pac);
      __asm__ __volatile__("autg %0, %1, %2" : : "r"(pac), "r"(lr), "r"(sp) :);
    }
#else
    (void)hasReturnAddrAuthCode;
#endif
    _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_IP, _UVRSD_UINT32, &lr);
  }
  return _URC_CONTINUE_UNWIND;
}

extern "C" _LIBUNWIND_EXPORT _swipr_Unwind_Reason_Code
__aeabi_unwind_cpp_pr0(_swipr_Unwind_State state, _swipr_Unwind_Control_Block *ucbp,
                       _swipr_Unwind_Context *context) {
  return unwindOneFrame(state, ucbp, context);
}

extern "C" _LIBUNWIND_EXPORT _swipr_Unwind_Reason_Code
__aeabi_unwind_cpp_pr1(_swipr_Unwind_State state, _swipr_Unwind_Control_Block *ucbp,
                       _swipr_Unwind_Context *context) {
  return unwindOneFrame(state, ucbp, context);
}

extern "C" _LIBUNWIND_EXPORT _swipr_Unwind_Reason_Code
__aeabi_unwind_cpp_pr2(_swipr_Unwind_State state, _swipr_Unwind_Control_Block *ucbp,
                       _swipr_Unwind_Context *context) {
  return unwindOneFrame(state, ucbp, context);
}

static _swipr_Unwind_Reason_Code
unwind_phase1(swipr_unw_context_t *uc, swipr_unw_cursor_t *cursor, _swipr_Unwind_Exception *exception_object) {
  ...
  for (bool handlerNotFound = true; handlerNotFound;) {
    ...
    if (_LIBUNWIND_TRACING_UNWINDING) {
      ...
      if ((__swipr_unw_get_proc_name(cursor, functionBuf, sizeof(functionBuf),
                               &offset) != UNW_ESUCCESS) ||
    ...
    if (frameInfo.handler != 0) {
      _swipr_Unwind_Personality_Fn p =
          (_swipr_Unwind_Personality_Fn)(long)(frameInfo.handler);
      _LIBUNWIND_TRACE_UNWINDING(
          "unwind_phase1(ex_ojb=%p): calling personality function %p",
          static_cast<void *>(exception_object),
          reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(p)));
      struct _swipr_Unwind_Context *context = (struct _swipr_Unwind_Context *)(cursor);
      exception_object->pr_cache.fnstart = frameInfo.start_ip;
      exception_object->pr_cache.ehtp =
          (_swipr_Unwind_EHT_Header *)frameInfo.unwind_info;
...
static _swipr_Unwind_Reason_Code unwind_phase2(swipr_unw_context_t *uc, swipr_unw_cursor_t *cursor,
  ...
  while (true) {
    ...
    // _swipr_Unwind_RaiseException or _swipr_Unwind_Resume).
    //
    // Resume only ever makes sense for 1 frame.
    _swipr_Unwind_State state =
        resume ? _US_UNWIND_FRAME_RESUME : _US_UNWIND_FRAME_STARTING;
    if (resume && frame_count == 1) {
      // On a resume, first unwind the _swipr_Unwind_Resume() frame. The next frame
      ...
      __swipr_unw_set_reg(cursor, UNW_REG_IP,
    ...
    if (frameInfo.handler != 0) {
      ...
      case _URC_INSTALL_CONTEXT:
        ...
        {
          // EHABI #7.4.1 says we need to preserve pc for when _swipr_Unwind_Resume
        ...
        // __swipr_unw_resume() only returns if there was an error.
        return _URC_FATAL_PHASE2_ERROR;
...
unwind_phase2_forced(swipr_unw_context_t *uc, swipr_unw_cursor_t *cursor,
                     _swipr_Unwind_Exception *exception_object, _swipr_Unwind_Stop_Fn stop,
                     void *stop_parameter) {
  ...
  while (!endOfStack) {
    ...
    swipr_unw_proc_info_t frameInfo;
    if (__swipr_unw_get_proc_info(cursor, &frameInfo) != UNW_ESUCCESS) {
      _LIBUNWIND_TRACE_UNWINDING("unwind_phase2_forced(ex_ojb=%p): __swipr_unw_get_proc_info "
    ...
    _swipr_Unwind_Reason_Code stopResult =
        (*stop)(1, action, exception_object->exception_class, exception_object,
                (_swipr_Unwind_Context *)(cursor), stop_parameter);
    _LIBUNWIND_TRACE_UNWINDING(
...
_swipr_Unwind_RaiseException(_swipr_Unwind_Exception *exception_object) {
  ...
  _swipr_Unwind_Reason_Code phase1 = unwind_phase1(&uc, &cursor, exception_object);
...
_LIBUNWIND_EXPORT void _swipr_Unwind_Complete(_swipr_Unwind_Exception* exception_object) {
...
/// When _swipr_Unwind_RaiseException() is in phase2, it hands control
...
/// in turn calls _swipr_Unwind_Resume_or_Rethrow().
_LIBUNWIND_EXPORT void
_swipr_Unwind_Resume(_swipr_Unwind_Exception *exception_object) {
  _LIBUNWIND_TRACE_API("_swipr_Unwind_Resume(ex_obj=%p)",
                       static_cast<void *>(exception_object));
  swipr_unw_context_t uc;
  swipr_unw_cursor_t cursor;
  __swipr_unw_getcontext(&uc);
  ...
  if (exception_object->unwinder_cache.reserved1)
    unwind_phase2_forced(
        ...
        (_swipr_Unwind_Stop_Fn)exception_object->unwinder_cache.reserved1,
...
_swipr_Unwind_GetLanguageSpecificData(struct _swipr_Unwind_Context *context) {
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  swipr_unw_proc_info_t frameInfo;
  uintptr_t result = 0;
  if (__swipr_unw_get_proc_info(cursor, &frameInfo) == UNW_ESUCCESS)
    result = (uintptr_t)frameInfo.lsda;
  _LIBUNWIND_TRACE_API(
      "_swipr_Unwind_GetLanguageSpecificData(context=%p) => 0x%llx",
...
ValueAsBitPattern(_swipr_Unwind_VRS_DataRepresentation representation,
                  const void *valuep) {
  uint64_t value = 0;
...
_LIBUNWIND_EXPORT _swipr_Unwind_VRS_Result
_swipr_Unwind_VRS_Set(_swipr_Unwind_Context *context, _swipr_Unwind_VRS_RegClass regclass,
                uint32_t regno, _swipr_Unwind_VRS_DataRepresentation representation,
                void *valuep) {
  _LIBUNWIND_TRACE_API("_swipr_Unwind_VRS_Set(context=%p, regclass=%d, reg=%d, "
                       "rep=%d, value=0x%llX)",
  ...
  switch (regclass) {
    case _UVRSC_CORE:
      ...
      return __swipr_unw_set_reg(cursor, (swipr_unw_regnum_t)(UNW_ARM_R0 + regno),
                           *(swipr_unw_word_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
    case _UVRSC_VFP:
      ...
      if (representation == _UVRSD_VFPX) {
        ...
        if (regno > 15)
          return _UVRSR_FAILED;
        __swipr_unw_save_vfp_as_X(cursor);
      } else {
        if (regno > 31)
          return _UVRSR_FAILED;
      }
      return __swipr_unw_set_fpreg(cursor, (swipr_unw_regnum_t)(UNW_ARM_D0 + regno),
                             *(swipr_unw_fpreg_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
#if defined(__ARM_WMMX)
    case _UVRSC_WMMXC:
      if (representation != _UVRSD_UINT32 || regno > 3)
        return _UVRSR_FAILED;
      return __swipr_unw_set_reg(cursor, (swipr_unw_regnum_t)(UNW_ARM_WC0 + regno),
                           *(swipr_unw_word_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
    case _UVRSC_WMMXD:
      if (representation != _UVRSD_DOUBLE || regno > 31)
        return _UVRSR_FAILED;
      return __swipr_unw_set_fpreg(cursor, (swipr_unw_regnum_t)(UNW_ARM_WR0 + regno),
                             *(swipr_unw_fpreg_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
#else
    ...
    case _UVRSC_PSEUDO:
      ...
      if (representation != _UVRSD_UINT32 || regno != 0)
        return _UVRSR_FAILED;
      return __swipr_unw_set_reg(cursor, (swipr_unw_regnum_t)(UNW_ARM_RA_AUTH_CODE),
                           *(swipr_unw_word_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
      break;
...
static _swipr_Unwind_VRS_Result
_swipr_Unwind_VRS_Get_Internal(_swipr_Unwind_Context *context,
                         _swipr_Unwind_VRS_RegClass regclass, uint32_t regno,
                         _swipr_Unwind_VRS_DataRepresentation representation,
                         void *valuep) {
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  switch (regclass) {
    case _UVRSC_CORE:
      if (representation != _UVRSD_UINT32 || regno > 15)
        return _UVRSR_FAILED;
      return __swipr_unw_get_reg(cursor, (swipr_unw_regnum_t)(UNW_ARM_R0 + regno),
                           (swipr_unw_word_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
    case _UVRSC_VFP:
      ...
      return __swipr_unw_get_fpreg(cursor, (swipr_unw_regnum_t)(UNW_ARM_D0 + regno),
                             (swipr_unw_fpreg_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
#if defined(__ARM_WMMX)
    case _UVRSC_WMMXC:
      if (representation != _UVRSD_UINT32 || regno > 3)
        return _UVRSR_FAILED;
      return __swipr_unw_get_reg(cursor, (swipr_unw_regnum_t)(UNW_ARM_WC0 + regno),
                           (swipr_unw_word_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
    case _UVRSC_WMMXD:
      if (representation != _UVRSD_DOUBLE || regno > 31)
        return _UVRSR_FAILED;
      return __swipr_unw_get_fpreg(cursor, (swipr_unw_regnum_t)(UNW_ARM_WR0 + regno),
                             (swipr_unw_fpreg_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
#else
    ...
    case _UVRSC_PSEUDO:
      ...
      if (representation != _UVRSD_UINT32 || regno != 0)
        return _UVRSR_FAILED;
      return __swipr_unw_get_reg(cursor, (swipr_unw_regnum_t)(UNW_ARM_RA_AUTH_CODE),
                           (swipr_unw_word_t *)valuep) == UNW_ESUCCESS
                 ? _UVRSR_OK
                 : _UVRSR_FAILED;
      break;
...
_LIBUNWIND_EXPORT _swipr_Unwind_VRS_Result
_swipr_Unwind_VRS_Get(_swipr_Unwind_Context *context, _swipr_Unwind_VRS_RegClass regclass,
                uint32_t regno, _swipr_Unwind_VRS_DataRepresentation representation,
                void *valuep) {
  _swipr_Unwind_VRS_Result result =
      _swipr_Unwind_VRS_Get_Internal(context, regclass, regno, representation,
                               valuep);
  _LIBUNWIND_TRACE_API("_swipr_Unwind_VRS_Get(context=%p, regclass=%d, reg=%d, "
...
_swipr_Unwind_VRS_Result
_swipr_Unwind_VRS_Pop(_swipr_Unwind_Context *context, _swipr_Unwind_VRS_RegClass regclass,
                uint32_t discriminator,
                _swipr_Unwind_VRS_DataRepresentation representation) {
  _LIBUNWIND_TRACE_API("_swipr_Unwind_VRS_Pop(context=%p, regclass=%d, "
  ...
  switch (regclass) {
    ...
    case _UVRSC_CORE: {
      ...
      bool poppedSP = false;
      uint32_t* sp;
      if (_swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_SP,
      ...
      for (uint32_t i = 0; i < 16; ++i) {
        ...
        if (regclass == _UVRSC_CORE && i == 13)
          poppedSP = true;
        if (_swipr_Unwind_VRS_Set(context, regclass, i,
                            _UVRSD_UINT32, &value) != _UVRSR_OK) {
          return _UVRSR_FAILED;
        }
      }
      if (!poppedSP) {
        return _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_SP,
                               _UVRSD_UINT32, &sp);
      }
      return _UVRSR_OK;
    }
...
#error "Unable to determine endianess"
#endif
        if (_swipr_Unwind_VRS_Set(context, regclass, i, representation, &value) !=
            _UVRSR_OK)
          return _UVRSR_FAILED;
      }
      if (representation == _UVRSD_VFPX)
        ++sp;
      return _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32,
                             &sp);
    }
    case _UVRSC_PSEUDO: {
      ...
      uint32_t *sp;
      if (_swipr_Unwind_VRS_Get(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32,
                          &sp) != _UVRSR_OK) {
        return _UVRSR_FAILED;
      }
      uint32_t pac = *sp++;
      _swipr_Unwind_VRS_Set(context, _UVRSC_CORE, UNW_ARM_SP, _UVRSD_UINT32, &sp);
      return _swipr_Unwind_VRS_Set(context, _UVRSC_PSEUDO, 0, _UVRSD_UINT32, &pac);
    }
  }
  _LIBUNWIND_ABORT("unsupported register class");
}
...
_LIBUNWIND_EXPORT _swipr_Unwind_Reason_Code
_swipr_Unwind_ForcedUnwind(_swipr_Unwind_Exception *exception_object, _swipr_Unwind_Stop_Fn stop,
                     void *stop_parameter) {
...
_swipr_Unwind_GetRegionStart(struct _swipr_Unwind_Context *context) {
  ...
  _LIBUNWIND_TRACE_API("_swipr_Unwind_GetRegionStart(context=%p) => 0x%llX",
                       static_cast<void *>(context), (long long)result);
  return result;
}
...
// is caught.
_LIBUNWIND_EXPORT void
_swipr_Unwind_DeleteException(_swipr_Unwind_Exception *exception_object) {
  _LIBUNWIND_TRACE_API("_swipr_Unwind_DeleteException(ex_obj=%p)",
                       static_cast<void *>(exception_object));
...
extern "C" _LIBUNWIND_EXPORT _swipr_Unwind_Reason_Code
__gnu_unwind_frame(_swipr_Unwind_Exception *exception_object,
                   struct _swipr_Unwind_Context *context) {
  (void)exception_object;
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  switch (__swipr_unw_step(cursor)) {
  case UNW_STEP_SUCCESS:
    return _URC_OK;
  case UNW_STEP_END:
    return _URC_END_OF_STACK;
