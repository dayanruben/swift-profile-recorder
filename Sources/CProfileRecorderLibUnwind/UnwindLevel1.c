...
// ARM EHABI does not specify _swipr_Unwind_{Get,Set}{GR,IP}().  Thus, we are
// defining inline functions to delegate the function calls to
// _swipr_Unwind_VRS_{Get,Set}().  However, some applications might declare the
    ...
    if (frameInfo.handler != 0) {
      ...
      _swipr_Unwind_Reason_Code personalityResult =
          (*p)(1, _UA_SEARCH_PHASE, exception_object->exception_class,
               exception_object, (struct _swipr_Unwind_Context *)(cursor));
...
// _URC_FATAL_PHASE2_ERROR.
    ...
    if (frameInfo.handler != 0) {
      ...
      case _URC_INSTALL_CONTEXT:
        ...
        __swipr_unw_phase2_resume(cursor, framesWalked);
        // __swipr_unw_phase2_resume() only returns if there was an error.
        return _URC_FATAL_PHASE2_ERROR;
      default:
...
_LIBUNWIND_EXPORT uintptr_t
_swipr_Unwind_GetGR(struct _swipr_Unwind_Context *context, int index) {
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  swipr_unw_word_t result;
  __swipr_unw_get_reg(cursor, index, &result);
  _LIBUNWIND_TRACE_API("_swipr_Unwind_GetGR(context=%p, reg=%d) => 0x%" PRIxPTR,
...
_LIBUNWIND_EXPORT void _swipr_Unwind_SetGR(struct _swipr_Unwind_Context *context, int index,
                                     uintptr_t value) {
  _LIBUNWIND_TRACE_API("_swipr_Unwind_SetGR(context=%p, reg=%d, value=0x%0" PRIxPTR
                       ")",
                       (void *)context, index, value);
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  __swipr_unw_set_reg(cursor, index, value);
}
...
_LIBUNWIND_EXPORT uintptr_t _swipr_Unwind_GetIP(struct _swipr_Unwind_Context *context) {
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  swipr_unw_word_t result;
  __swipr_unw_get_reg(cursor, UNW_REG_IP, &result);
...
/// such as setting where the landing pad is, so _swipr_Unwind_Resume() will
/// start executing in the landing pad.
_LIBUNWIND_EXPORT void _swipr_Unwind_SetIP(struct _swipr_Unwind_Context *context,
                                     uintptr_t value) {
  _LIBUNWIND_TRACE_API("_swipr_Unwind_SetIP(context=%p, value=0x%0" PRIxPTR ")",
                       (void *)context, value);
  swipr_unw_cursor_t *cursor = (swipr_unw_cursor_t *)context;
  __swipr_unw_set_reg(cursor, UNW_REG_IP, value);
}
