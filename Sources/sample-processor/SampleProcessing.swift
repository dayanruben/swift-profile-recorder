...
func process(_ sample: Sample, _ dynamicLibs: [DynamicLibMapping]) throws {
    ...
    //print(dynamicLibs.map { $0.description }.joined(separator: "\n"))
    ...
    try Data(symbolizerInput.utf8).write(to: inputURL)
    ...
    print("foo     \(sample.pid)/\(sample.tid)     \(sample.timeSec).\(sample.timeNSec):    1001001 cpu-clock:pppH:")
    let lines = String(decoding: try Data(contentsOf: outputURL), as: UTF8.self).components(separatedBy: "\n")
    for index in 0..<sample.stack.count {
